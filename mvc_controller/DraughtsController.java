...
public class DraughtsController implements Runnable {
    ...
    private void update(int from, int to) {
        TokenModel fromToken = token.getToken(from);
        TokenModel toToken = token.getToken(to);
    ...
    public void makeMove(TokenModel from, TokenModel to) {
    ...
    private void selected(TokenModel t) {
        t.setSelected(true);
        selectedTokens.add(t);
        getPlayableTokens(t);
    }

    private void getPlayableTokens(TokenModel t) {
        playableTokens.clear();
        playableTokens = token.playableToken(t);
        token.play();
    }

    public void selectToken(TokenModel t) {
    ...
    public void deselectToken() {
        for (TokenModel token : selectedTokens) token.setSelected(false);
        selectedTokens.clear();
        for (TokenModel token : playableTokens) { token.moveable(false); }
        playableTokens.clear();
        token.play();
    }
    ...
    private void checkJump(TokenModel from, TokenModel to) {
        if (Math.abs(from.getTokenRow() - to.getTokenRow()) == 2) {
            ...
            TokenModel jumpedToken = token.getToken((r * 8) + c + 1);
    ...
    private void checkKing(TokenModel from, TokenModel tokenPlayed) {
        if (from.king()) {
